...
export default class AddonZmodemWasm {
   ...
   */
  sendLogToServer (level: string, ...args: unknown[]): void {
  ...
  zmodemAttach (ctx: { socket: WebSocket, term: Terminal, onDetect?: (type: 'receive' | 'send') => void }): void {
  ...
  _consumeInternal (data: ArrayBuffer | string): void {
  ...
  async sendFile (file: File): Promise<void> {
    ...
    try {
      ...
      this.term?.writeln('\r\nZMODEM: Failed to start send - ' + String(e))
      this.sender = null
    }
  }

  handleSender (data: ArrayBuffer | Uint8Array | string): void {
  ...
  pumpSender (): boolean {
    ...
    const flushOutgoing = (): void => {
      ...
      if (outgoingChunks.length === 1) {
        ...
        this.socket?.send(new Blob(outgoingChunks as BlobPart[]))
      }
    ...
    try {
      ...
      if (outgoing !== null && outgoing !== undefined && outgoing.length > 0) {
        ...
        if (fileRequest !== null && fileRequest !== undefined) {
          ...
          if ((this._fileBuffer != null) &&
            ...
            if (outgoingInner !== null && outgoingInner !== undefined && outgoingInner.length > 0) {
              outgoingChunks.push(outgoingInner)
              totalOutgoingSize += outgoingInner.length
          ...
          if ((this.sendingFile != null) && !this._reading) {
            flushOutgoing() // Flush before async break
            this._reading = true // Lock
            void this.loadBufferAndFeed(start, length)
  ...
  async loadBufferAndFeed (offset: number, length: number): Promise<void> {
    if (this.sender === null || this.sendingFile === null) {
      this._reading = false
      return
    }
    try {
  ...
  startReceiver (initialData: Uint8Array): void {
    ...
    try {
      ...
      this.term?.writeln('\r\nZMODEM: Failed to start receiver - ' + String(e))
      this.receiver = null
    }
  }

  handleReceiver (data: ArrayBuffer | Uint8Array | string): void {
    ...
    while (offset < u8.length && loopCount++ < 1000) {
      ...
      try {
        ...
        this.error('Receiver error:', e, err.details)
        this.term?.writeln('\r\nZMODEM: Error ' + String(e) + ' ' + JSON.stringify(err.details ?? {}))
        this.receiver = null
  ...
  saveFile (): void {
    ...
    try {
      const blob = new Blob(this.currentFile.data as BlobPart[], { type: 'application/octet-stream' })
      ...
      this.term?.writeln('\r\nZMODEM: Failed to save file - ' + String(e))
    }
    this.currentFile = null
  }
}
